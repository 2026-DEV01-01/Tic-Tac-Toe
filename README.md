...
* **Java 21 Optimized:** Utilizes modern Java features such as `var`, `String.formatted()`, immutable collections (`List.of`), and functional streams.
...
4. **Validation Engine (`GameValidationRule`):** Utilizes the Rules Pattern. A central orchestrator (`TicTacToeValidator`) sequentially routes requests through specific, isolated rules (e.g., `BoardStateRule`, `BoundsAndAvailabilityRule`).
